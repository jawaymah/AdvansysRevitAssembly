...
namespace AdvansysRevitAssembly.Commands
{
    ...
    public class ConveyorADV1Command : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                Family family = FindFamilyByName(doc, "ADV-1006");
                if (family == null)
                {
                    using (Transaction t = new Transaction(doc, "Load Family Instance"))
                    {
                        ...
                        string familyPath = new Uri(Path.Combine(UIConstants.ButtonFamiliesFolder, "ADV-1006.rfa"), UriKind.Absolute).AbsolutePath;
