...
namespace AdvansysRevitAssembly.Commands
{
    ...
    public class Conveyor2Command : IExternalCommand
    {
        public Result Execute2(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                {
                    t.Start();
                    //doc.LoadFamilySymbol(@"C:\Users\Jemmy\AppData\Roaming\Autodesk\Revit\Addins\2024\roller7.rfa", "roller7", out FamilySymbol familySymbol);
                    //doc.LoadFamilySymbol(@"C:\Users\Jemmy\AppData\Roaming\Autodesk\Revit\Addins\2024\conveyor.rfa", "conveyor", out FamilySymbol familySymbol);
                    //pdData.LargeImage = new BitmapImage();
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                Document doc = commandData.Application.ActiveUIDocument.Document;
                Family family = FindFamilyByName(doc, "curve");
                if (family == null)
                {
                    using (Transaction t = new Transaction(doc, "Load Family Instance"))
                    {
                        ...
                        string familyPath = new Uri(Path.Combine(UIConstants.ButtonFamiliesFolder, "curve.rfa"), UriKind.Absolute).AbsolutePath;
