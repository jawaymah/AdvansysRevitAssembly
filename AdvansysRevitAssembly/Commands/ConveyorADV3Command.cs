...
namespace AdvansysRevitAssembly.Commands
{
    ...
    public class ConveyorADV3Command : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                Family family = FindFamilyByName(doc, "ADV-3005");
                if (family == null)
                {
                    using (Transaction t = new Transaction(doc, "Load Family Instance"))
                    {
                        ...
                        string familyPath = new Uri(Path.Combine(UIConstants.ButtonFamiliesFolder, "ADV-3005.rfa"), UriKind.Absolute).AbsolutePath;
